...
# modify:   hiems MMXXI
# -----------------------------------------------------------------------------
""" This is the main module of Northwind in hot sauce."""
import sys
...
__description__ = """ is an application developed to show the differences in programming and rendering, for example in layout, between wxPython and Tkinter platforms.\nIt is based on Microsoft's famous Northwind database, hence the name of the project.\n"""
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title("Northwind in hot sauce")
        self.engine.set_style("clam")
        self.set_icon()
        self.set_info()
    ...
    def set_info(self,):
        msg = "{0} {1}\nauthor: {2}\ncopyright: {3}\ncredits: {4}\nlicense: {5}\nversion: {6}\
               \nmaintainer: {7}\nemail: {8}\ndate: {9}\nstatus: {10}"
        info = msg.format(self.title(),__description__, __author__, __copyright__, __credits__, __license__, __version__, __maintainer__, __email__, __date__, __status__)
        self.info = info
